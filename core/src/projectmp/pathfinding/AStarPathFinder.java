package projectmp.pathfinding;

import projectmp.blocks.Block;
import projectmp.pathfinding.heuristics.ClosestHeuristic;
...
public class AStarPathFinder implements PathFinder {
	...
	public Block getRoom(int x, int y) {
		if (map instanceof World) {
			return ((World) map).getBlock(x, y);
		}
		return null;
	}
	...
	public Path findPath(Mover mover, int sx, int sy, int tx, int ty) {
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			if (System.currentTimeMillis() - time > 150l) return null;
			...
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
					...
					int xp = x + current.x;
					int yp = y + current.y;
					// System.out.println("pf: " + isValidLocation(mover,
					// current.x, current.y, sx, sy, xp, yp));
					if (isValidLocation(mover, current.x, current.y, sx, sy, xp, yp)) {
	 ...
	 */
	protected boolean isValidLocation(Mover mover, int cx, int cy, int sx, int sy, int x, int y) {
		...
		if (!map.canMoveDirectly(mover, cx, cy, x, y)) return false;
