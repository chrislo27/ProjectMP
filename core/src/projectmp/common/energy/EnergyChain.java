...
public class EnergyChain {
	...
	private static int traverse(int energy, int startX, int startY, int bannedX, int bannedY,
		...
		while (true) {
			...
			if (currentBlock instanceof BlockPowerHandler) {
				IPowerHandler phandler = (IPowerHandler) (((BlockPowerHandler) currentBlock)
						.getPowerHandler(world, currentX, currentY));
	...
	private static int[] rationPower(int energy, int startX, int startY, int bannedX, int bannedY,
			World world, int[] array) {
