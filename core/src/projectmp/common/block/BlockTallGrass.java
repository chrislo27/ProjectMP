...
//		super.render(renderer, x, y);
	...
	public void tickUpdate(World world, int x, int y){
		...
		for(int i = 0; i < nearby.size(); i++){
			...
			if(MathHelper.intersects(bounds.x, bounds.y, bounds.width, bounds.height, e.visualX, e.visualY, e.sizex, e.sizey)){
				
				int m = world.getMeta(x, y);
				world.setMeta((int) MathUtils.clamp(m + (e.velox * World.tilesizey * 0.05f), -World.tilesizex / 2, World.tilesizex / 2), x, y);
			}
		}
	}
