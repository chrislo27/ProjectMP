...
public class ItemMineable extends Item {
	 ...
	 * <br>
	 * A tool with 1.0 effectiveness can mine a block with 0.5 hardness in 0.5 seconds.
	 * <br>
	 * <br>
	 * That means the time it takes to mine a block is effectiveness / hardness (plus compensation for block recede).
	 */
	...
	public void onUsing(World world, EntityPlayer player, int slot, int x, int y) {
		if (world.isServer == false) {
			...
			world.setBreakingProgress(x, y, progress
					+ ((effectiveness / world.getBlock(x, y).getHardness()) / Main.TICKS)
					+ (World.BLOCK_RECEDE / Main.TICKS));
